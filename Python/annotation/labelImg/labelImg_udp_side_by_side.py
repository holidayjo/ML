...
# CHECK IF THERE IS CLASSES.TXT FILE IN THE FOLDER!!!
import argparse
import codecs
import os.path
import platform
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.base_image = None  # To store the unmodified original
        ...
        # --- START CHANGES ---
        # Create a second canvas for the original image
        self.canvas_ref = Canvas(parent=self)
        self.canvas_ref.setEnabled(True) # Set to False if you want it strictly read-only

        # Create a scroll area for the reference canvas
        scroll_ref = QScrollArea()
        scroll_ref.setWidget(self.canvas_ref)
        scroll_ref.setWidgetResizable(True)

        # Use QSplitter to show them side-by-side
        self.splitter = QSplitter(Qt.Horizontal)
        self.splitter.addWidget(scroll)       # Left side (Editing)
        self.splitter.addWidget(scroll_ref)   # Right side (Reference)
        ...
        edit_mode = action(get_str('editBox'), self.set_edit_mode,
                           'T', 'edit', get_str('editBoxDetail'), enabled=False)
        # ---------------------
        ...
        # --- START CHANGES ---
        # CLECH / CLAHE Action
        self.clahe_action = QAction("CLECH Equalization", self)
        self.clahe_action.setCheckable(True)
        ...
        add_actions(self.menus.view, (
            ...
            self.clahe_action,  # <--- Add this line here
    ...
    # -------------------------------------------------------------
    # CUSTOM COLOR FUNCTION (SPECIFIC FOR U, D, P)
    # -------------------------------------------------------------
    # -------------------------------------------------------------
    # CUSTOM COLOR FUNCTION (SPECIFIC FOR U, D, P)
    # -------------------------------------------------------------
    def get_distinct_color(self, label):
        # We handle U, D, P classes here:
        # The 4th number (128) is the Transparency (0-255)
        color_map = {
            "U": QColor(255, 0, 0, 128),    # Red, 50% transparent
            "D": QColor(0, 255, 0, 128),    # Green, 50% transparent
            "P": QColor(0, 0, 255, 128),    # Blue, 50% transparent
        }
        
        if label in color_map:
            return color_map[label]
        else:
            # Also apply transparency to auto-generated colors
    ...
    #     # We handle U, D, P classes here:
    #     color_map = {
    #         "U": QColor(255, 0, 0),    # Red
    #         "D": QColor(0, 255, 0),    # Green
    #         "P": QColor(0, 0, 255),    # Blue
    #     }
        
    #     if label in color_map:
    #         return color_map[label]
    #     else:
    #         return generate_color_by_text(label)
    ...
    def edit_label(self):
        ...
        if text is not None:
            item.setText(text)
            # UPDATED: Use custom color
            item.setBackground(self.get_distinct_color(text))
            self.set_dirty()
    ...
    def add_label(self, shape):
        ...
        # UPDATED: Use custom color
        item.setBackground(self.get_distinct_color(shape.label))
    ...
    def load_labels(self, shapes):
        ...
        for label, points, line_color, fill_color, difficult in shapes:
            ...
            # UPDATED: Force custom colors
            shape.line_color = self.get_distinct_color(label)
            shape.fill_color = self.get_distinct_color(label)
    ...
    def label_item_changed(self, item):
        ...
        if label != shape.label:
            ...
            # UPDATED: Use custom color
            shape.line_color = self.get_distinct_color(shape.label)
            self.set_dirty()
        else:  # User probably changed item visibility
            self.canvas.set_shape_visible(shape, item.checkState() == Qt.Checked)
    ...
    def new_shape(self):
        ...
        if text is not None:
            ...
            # UPDATED: Use custom color
            generate_color = self.get_distinct_color(text)
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            # --- START CHANGES ---
            self.base_image = image  # Store the pure original
            self.file_path = unicode_file_path
            
            # This function will handle setting self.image to either 
            # the original or the CLECH version, and loading it into the canvas
            ...
            # --- MODIFIED: COMMENTED OUT THESE LINES TO STOP AUTO-SELECTION ---
            # if self.label_list.count():
            #     self.label_list.setCurrentItem(self.label_list.item(self.label_list.count() - 1))
            #     self.label_list.item(self.label_list.count() - 1).setSelected(True)
            
            # --- ADDED: EXPLICITLY CLEAR SELECTION ---
            self.label_list.clearSelection()
    ...
    def update_displayed_image(self):
        ...
        # 1. Prepare the image for the Main Canvas (Left)
        if self.clahe_action.isChecked():
            self.image = self.apply_clahe(self.base_image)
        else:
            self.image = self.base_image

        self.canvas.load_pixmap(QPixmap.fromImage(self.image))

        # 2. Ensure Reference Canvas (Right) always shows Original
        if hasattr(self, 'canvas_ref'):
            self.canvas_ref.load_pixmap(QPixmap.fromImage(self.base_image))

    def apply_clahe(self, qimg):
        """Converts QImage to LAB, applies CLAHE to L channel, returns QImage."""
        # Convert QImage to RGB format for OpenCV
        tmp = qimg.convertToFormat(QImage.Format_RGB888)
        ...
        return QImage(final.data, width, height, 3 * width, QImage.Format_RGB888).copy()
    # --- END NEW METHODS ---
    ...
    def paint_canvas(self):
        ...
        self.canvas.update()
        # --- START CHANGES ---
        self.canvas_ref.scale = self.canvas.scale
        self.canvas_ref.adjustSize()
    ...
    def scale_fit_window(self):
        ...
        # --- START CHANGES (Replace self.centralWidget() with self.scroll_area) ---
