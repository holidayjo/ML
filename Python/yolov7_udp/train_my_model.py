...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Freeze.  freezing specific layers in the model during training.
    # print("freeze =", freeze)    # freeze = [0]
    freeze = [f'model.{x}.' for x in (freeze if len(freeze) > 1 else range(freeze[0]))]  # parameter names to freeze (full or partial)
    # print("freeze =", freeze)    # freeze = []
    ...
    # Optimizer
    nbs        = 64  # Defines the nominal batch size used to normalize certain hyperparameters.
    accumulate = max(round(nbs / total_batch_size), 1)  # accumulate loss before optimizing
                                                        # This calculates how many batches to accumulate before performing an optimization step.
                                                        # It’s a strategy for handling large batch sizes by updating the model weights less frequently
                                                        # but based on multiple smaller batches.
    ...
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups
                                # pg0: not be subject to weight decay
                                # pg1: have weight decay applied.
                                # pg2: biases.
    # ii = 0
    for k, v in model.named_modules():
        ...
        # if ii == 9:
        #     break
        # The code below checks if the model has specific components 
        # (e.g., implicit layers, attention mechanisms, or RepVGG blocks)
        # and adds them to the appropriate parameter group.
        if hasattr(v, 'im'):
    ...
    # print("pg0 =", len(pg0)) # pg0 (other): The number of parameters that don't use weight decay (e.g., batch normalization weights, implicit layers, etc.).
    # print("pg1 =", len(pg1)) # pg1 (conv.weight): The number of convolution weights (or any weights that use weight decay).
    # print("pg2 =", len(pg2)) # pg2 (biases): The number of bias parameters.
    # ii = 0
    # for iii in pg1:
    #     ii += 1
    #     print("ii={}, iii={}".format(ii, iii.size()))
    #     break
    ...
    optimizer.add_param_group({'params': pg2})                                       # add pg2 (biases) -> optimized w/o weight_decay
    logger.info('Optimizer groups: %g .bias, %g conv.weight, %g other' % (len(pg2), len(pg1), len(pg0)))
    del pg0, pg1, pg2

    # Scheduler https://arxiv.org/pdf/1812.01187.pdf
    # https://pytorch.org/docs/stable/_modules/torch/optim/lr_scheduler.html#OneCycleLR
    # These decrease the learning rate.
    if opt.linear_lr:
